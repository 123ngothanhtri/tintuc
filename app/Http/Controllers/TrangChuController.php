...
{
    function trangchu(){
        $tt=BaiViet::where('anhien_baiviet','1')->orderBy('id_baiviet', 'desc')->get();
        $lt=LoaiBaiViet::all();
    ...
    function loc($id){
        $tt=BaiViet::orWhere('id_loaibaiviet',$id)->where('anhien_baiviet','1')->orderBy('id_baiviet', 'desc')->get();
        $lt=LoaiBaiViet::all();
        $xn=$id;
    ...
    function timkiembaiviet(Request $r){
        $tukhoa=$r->tktt;
        $lt=LoaiBaiViet::all();
        $tt=BaiViet::where('tieude_baiviet','like',"%$tukhoa%")->get();//->stake(30)->paginate(5);
